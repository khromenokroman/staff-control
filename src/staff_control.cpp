...
StaffControl::StaffControl() {
    openlog("StaffControll", LOG_PID | LOG_CONS, LOG_USER);
...
void StaffControl::run() {
    ...
    m_server.Get("/employees", [this](const httplib::Request &req, httplib::Response &res) {
        syslog(LOG_INFO, "Поступил запрос('/employees') от %s:%d на %s:%d", req.remote_addr.c_str(), req.remote_port,
               req.local_addr.c_str(), req.local_port);
        try {
    ...
    m_server.Get("/add", [this](const httplib::Request &req, httplib::Response &res) {
        syslog(LOG_INFO, "Поступил запрос('/add-get') от %s:%d на %s:%d", req.remote_addr.c_str(), req.remote_port,
               req.local_addr.c_str(), req.local_port);
    ...
    m_server.Post("/add", [this](const httplib::Request &req, httplib::Response &res) {
        syslog(LOG_INFO, "Поступил запрос('/add-post') от %s:%d на %s:%d", req.remote_addr.c_str(), req.remote_port,
               req.local_addr.c_str(), req.local_port);
        try {
    ...
    m_server.Get("/export", [this](const httplib::Request &req, httplib::Response &res) {
        syslog(LOG_INFO, "Поступил запрос('/export') от %s:%d на %s:%d", req.remote_addr.c_str(), req.remote_port,
               req.local_addr.c_str(), req.local_port);
        try {
            export_to_kassa();
    ...
    if (!m_server.listen("0.0.0.0", m_config.port)) {
        throw std::runtime_error("Ошибка при выполнении команды listen");
    }
}

int StaffControl::random_int(int from, int to) const {
...
Users StaffControl::load_users() const {
    std::ifstream f(m_config.path_db);
    if (!f.is_open()) {
        auto err = errno;
...
void StaffControl::save_users(Users const &users) {
    ::nlohmann::json const j = users;

    std::ofstream f(m_config.path_db);
    if (!f.is_open()) {
        auto err = errno;
        throw std::runtime_error(::fmt::format("Ошибка открытия файла БД '{}': {}", m_config.path_db, strerror(err)));
    }
...
std::string StaffControl::html_escape(std::string_view s) const {
    std::string out;
    out.reserve(s.size());
    for (char c: s) {
        switch (c) {
...
std::string StaffControl::build_error_page(std::string_view message) const {
...
std::string StaffControl::build_index_page() const {
    return R"html(
<!doctype html>
<html lang="ru">
<head>
...
std::string StaffControl::build_add_page() const {
    return R"html(
<!doctype html>
<html lang="ru">
<head>
...
std::string StaffControl::build_employees_page(const Users &users) const {
    std::ostringstream out;
    out << R"html(
<!doctype html>
<html lang="ru">
...
std::string StaffControl::build_export_page() const {
    return R"html(
<!doctype html>
<html lang="ru">
<head>
